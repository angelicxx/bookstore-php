...
class Basket {
    ...
    public static function add($id) {
        if (!isset(self::$items[$id])) {
            self::$items[$id] = 1; // Количество по умолчанию = 1
        } else {
            self::$items[$id]++; // Увеличиваем количество
        }
        self::save();
    }
    ...
    public static function clear() {
        self::$items = []; // Очищаем массив корзины
        setcookie('basket', '', time() - 3600, '/'); // Удаляем куки корзины
    }
}
